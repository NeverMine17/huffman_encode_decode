/*************************************************************************
> File Name: h_enc.cpp
> Created Time: 2017-11-02 22:43:44
************************************************************************/
...
static inline char bits(int num, int k, int j) {
    return static_cast<char>((num >> k) & ~(~0 << j));
}
...
void print_result(int code[], const int bits_count[]) {
    int i, j;
...
int save_tree_file(FILE *fout) {
    ...
    int ret;
    int write_cnt = 0;
    auto f_pos = static_cast<int>(ftell(fout));
    assert(f_pos == 0);
    ...
    for (i = 0; i < COUNT; ++i) {
        ...
        if (g_bitCount[i]) {
            ...
            len += sizeof(*g_code);
            ret = static_cast<int>(fwrite(buf, 1, static_cast<size_t>(len), fout));
            if (ret != len) {
...
int encode_file(FILE *fout, const unsigned char *msg, int length) {
    ...
    for (i = 0; i < length; ++i) {
        ...
        assert(pos <= 40);
        len = static_cast<unsigned int>(pos / 8);
        if (len) {
...
int main(int argc, char **argv) {
    ...
    fseek(fp, 0, SEEK_END);
    auto file_length = static_cast<int>(ftell(fp));
    fseek(fp, 0, SEEK_SET);
    printf("file length:%d byte(s)\n", file_length);
    msg = new unsigned char[file_length];
    size_t read_size = fread(msg, 1, static_cast<size_t>(file_length), fp);
    if (read_size == 0) {
